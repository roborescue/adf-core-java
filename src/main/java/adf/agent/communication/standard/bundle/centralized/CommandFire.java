...
public class CommandFire extends StandardMessage {
  ...
  public CommandFire( boolean isRadio, int from, int ttl, @Nonnull BitStreamReader bitStreamReader ) {
    ...
    this.rawToID = ( bitStreamReader.getBits( 1 ) == 1 )
        ? bitStreamReader.getBits( SIZE_TO )
        : -1;
  ...
  public BitOutputStream toBitOutputStream() {
    ...
    if ( this.commandToID != null ) {
      bitOutputStream.writeBitsWithExistFlag( this.commandToID.getValue(),
          SIZE_TO );
    ...
    if ( this.commandTargetID != null ) {
      bitOutputStream.writeBitsWithExistFlag( this.commandTargetID.getValue(),
          SIZE_TARGET );
  ...
  public String getCheckKey() {
    String toIDValue = ( this.broadcast ? "broadcast"
        : Objects.requireNonNull( this.getToID() ).toString() );
    EntityID tid = this.getTargetID();
    String tidValue = ( tid == null ? "null" : tid.toString() );
    return getClass().getCanonicalName() + " > to:" + toIDValue + " target:"
        + tidValue + " action:" + this.myAction;
  }
}