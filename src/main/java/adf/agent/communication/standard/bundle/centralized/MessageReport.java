...
public class MessageReport extends StandardMessage {
  ...
  public MessageReport( boolean isRadio, boolean isDone, boolean isBroadcast, @Nullable EntityID fromID ) {
    this( isRadio, StandardMessagePriority.NORMAL, isDone, isBroadcast,
        fromID );
  }
  ...
  public MessageReport( boolean isRadio, int from, int ttl, @Nonnull BitStreamReader bitStreamReader ) {
    ...
    this.rawReportFromID = ( bitStreamReader.getBits( 1 ) == 1 )
        ? bitStreamReader.getBits( SIZE_FROM )
        : -1;
  }
  ...
  public EntityID getFromID() {
    if ( this.reportFromID == null ) {
      if ( this.rawReportFromID != -1 )
        this.reportFromID = new EntityID( this.rawReportFromID );
    }
  ...
  public BitOutputStream toBitOutputStream() {
    ...
    bitOutputStream.writeBits( ( this.reportBroadcast ? 1 : 0 ),
        SIZE_BROADCAST );
    if ( this.reportFromID != null ) {
      bitOutputStream.writeBitsWithExistFlag( this.reportFromID.getValue(),
          SIZE_FROM );
  ...
  public String getCheckKey() {
    return getClass().getCanonicalName() + " isBroadcast:" + this.isBroadcast()
