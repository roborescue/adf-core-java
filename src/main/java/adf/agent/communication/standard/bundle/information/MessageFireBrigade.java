...
public class MessageFireBrigade extends StandardMessage {
  ...
  public static final int  ACTION_EXTINGUISH = 3;
  public static final int  ACTION_REFILL     = 4;
  ...
  public MessageFireBrigade( boolean isRadio, @Nonnull FireBrigade fireBrigade, int action, @Nullable EntityID target ) {
    this( isRadio, StandardMessagePriority.NORMAL, fireBrigade, action,
        target );
  }
  ...
  public MessageFireBrigade( boolean isRadio, StandardMessagePriority sendingPriority, @Nonnull FireBrigade fireBrigade, int action, @Nullable EntityID target ) {
    ...
    this.humanBuriedness = fireBrigade.isBuriednessDefined()
        ? fireBrigade.getBuriedness()
        : -1;
    this.humanDamage = fireBrigade.isDamageDefined() ? fireBrigade.getDamage()
        : -1;
    this.humanPosition = fireBrigade.isPositionDefined()
        ? fireBrigade.getPosition()
        : null;
    this.myTargetID = target;
    this.myAction = action;
    this.fireBrigadeWater = fireBrigade.isWaterDefined()
        ? fireBrigade.getWater()
        : -1;
  }
  ...
  public MessageFireBrigade( boolean isRadio, int from, int ttl, @Nonnull BitStreamReader bitStreamReader ) {
    ...
    this.humanHP = ( bitStreamReader.getBits( 1 ) == 1 )
        ? bitStreamReader.getBits( SIZE_HP )
        : -1;
    this.humanBuriedness = ( bitStreamReader.getBits( 1 ) == 1 )
        ? bitStreamReader.getBits( SIZE_BURIEDNESS )
        : -1;
    this.humanDamage = ( bitStreamReader.getBits( 1 ) == 1 )
        ? bitStreamReader.getBits( SIZE_DAMAGE )
        : -1;
    this.rawHumanPosition = ( bitStreamReader.getBits( 1 ) == 1 )
        ? bitStreamReader.getBits( SIZE_POSITION )
        : -1;
    this.rawTargetID = ( bitStreamReader.getBits( 1 ) == 1 )
        ? bitStreamReader.getBits( SIZE_TARGET )
        : -1;
    this.myAction = bitStreamReader.getBits( SIZE_ACTION );
    this.fireBrigadeWater = ( bitStreamReader.getBits( 1 ) == 1 )
        ? bitStreamReader.getBits( SIZE_WATER )
        : -1;
  }
  ...
  public EntityID getTargetID() {
    if ( this.myTargetID == null ) {
      if ( this.rawTargetID != -1 )
        this.myTargetID = new EntityID( this.rawTargetID );
    }
  ...
  public BitOutputStream toBitOutputStream() {
    ...
    if ( this.humanBuriedness != -1 ) {
      bitOutputStream.writeBitsWithExistFlag( this.humanBuriedness,
          SIZE_BURIEDNESS );
    } else {
      bitOutputStream.writeNullFlag();
    }
    ...
    if ( this.humanPosition != null ) {
      bitOutputStream.writeBitsWithExistFlag( this.humanPosition.getValue(),
          SIZE_POSITION );
    } else if ( this.rawHumanPosition != -1 ) {
      bitOutputStream.writeBitsWithExistFlag( this.rawHumanPosition,
          SIZE_POSITION );
    } else {
      bitOutputStream.writeNullFlag();
    }
    if ( this.myTargetID != null ) {
      bitOutputStream.writeBitsWithExistFlag( this.myTargetID.getValue(),
          SIZE_TARGET );
    ...
    if ( this.fireBrigadeWater != -1 ) {
      bitOutputStream.writeBitsWithExistFlag( this.fireBrigadeWater,
          SIZE_WATER );
    } else {
      bitOutputStream.writeNullFlag();
    }
  ...
  public String getCheckKey() {
    ...
    return getClass().getCanonicalName() + " > agent:"
        + this.getAgentID().getValue() + " target:" + tidValue;
  }
}