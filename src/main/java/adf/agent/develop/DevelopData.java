...
public final class DevelopData {
  ...
  public static final String         DEFAULT_FILE_NAME = System.getProperty(
      "user.dir" ) + File.separator + "data" + File.separator + "develop.json";
  ...
  private void setRawData( @Nonnull String rawData, boolean isBase64 ) {
    ...
    String data = ( isBase64
        ? new String( Base64.getDecoder().decode( rawData ) )
        : rawData );
    ...
    try {
      ...
      ConsoleOutput.out( ConsoleOutput.State.WARN,
          "DevelopData input is invalid : " + data );
    } catch ( IOException e ) {
      e.printStackTrace();
    }
  ...
  private void setDataFile( @Nonnull String developDataFileName ) {
    ...
    if ( developDataFileName.equals( DEFAULT_FILE_NAME )
        && !( file.isFile() ) ) {
      return;
    }
    ...
    try {
      rawData = Files
          .lines( Paths.get( file.getPath() ), Charset.forName( "UTF-8" ) )
          .collect(
