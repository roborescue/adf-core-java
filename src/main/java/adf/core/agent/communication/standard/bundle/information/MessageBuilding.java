...
public class MessageBuilding extends StandardMessage {
  ...
  public MessageBuilding(boolean isRadio, StandardMessagePriority sendingPriority, @Nonnull Building building) {
    ...
    this.buildingBrokenness = building.isBrokennessDefined()
        ? building.getBrokenness()
        : -1;
    this.buildingFieryness = building.isFierynessDefined()
        ? building.getFieryness()
        : -1;
    this.buildingTemperature = building.isTemperatureDefined()
        ? building.getTemperature()
        : -1;
  }
  ...
  public MessageBuilding(boolean isRadio, int from, int ttl, @Nonnull BitStreamReader bitStreamReader) {
    ...
    this.buildingBrokenness = (bitStreamReader.getBits(1) == 1)
        ? bitStreamReader.getBits(SIZE_BROKENNESS)
        : -1;
    this.buildingFieryness = (bitStreamReader.getBits(1) == 1)
        ? bitStreamReader.getBits(SIZE_FIERYNESS)
        : -1;
    this.buildingTemperature = (bitStreamReader.getBits(1) == 1)
        ? bitStreamReader.getBits(SIZE_TEMPERATURE)
        : -1;
  }
  ...
  public BitOutputStream toBitOutputStream() {
    ...
    if (this.buildingBrokenness != -1) {
      bitOutputStream.writeBitsWithExistFlag(this.buildingBrokenness,
          SIZE_BROKENNESS);
    } else {
    ...
    if (this.buildingTemperature != -1) {
      bitOutputStream.writeBitsWithExistFlag(buildingTemperature,
          SIZE_TEMPERATURE);
    } else {
      bitOutputStream.writeNullFlag();
    }
  ...
  public String getCheckKey() {
    return getClass().getCanonicalName() + " > building:"
