...
public class MessageRoad extends StandardMessage {
  ...
  public MessageRoad(boolean isRadio, @Nonnull Road road, @Nullable Blockade blockade, @Nullable Boolean isPassable, boolean isSendBlockadeLocation) {
    this(isRadio, StandardMessagePriority.NORMAL, road, blockade, isPassable,
        isSendBlockadeLocation);
  }
  ...
  public MessageRoad(boolean isRadio, StandardMessagePriority sendingPriority, @Nonnull Road road, @Nullable Blockade blockade, @Nullable Boolean isPassable, boolean isSendBlockadeLocation) {
    ...
    if (blockade != null) {
      ...
      this.blockadeRepairCost = blockade.isRepairCostDefined()
          ? blockade.getRepairCost()
          : -1;
      this.blockadeX = (isSendBlockadeLocation && blockade.isXDefined())
          ? blockade.getX()
          : null;
      this.blockadeY = (isSendBlockadeLocation && blockade.isYDefined())
          ? blockade.getY()
          : null;
    } else {
  ...
  public MessageRoad(boolean isRadio, int from, int ttl, @Nonnull BitStreamReader bitStreamReader) {
    ...
    this.rawBlockadeID = (bitStreamReader.getBits(1) == 1)
        ? bitStreamReader.getBits(SIZE_BLOCKADEID)
        : -1;
    this.blockadeRepairCost = (bitStreamReader.getBits(1) == 1)
        ? bitStreamReader.getBits(SIZE_COST)
        : -1;
    this.blockadeX = (bitStreamReader.getBits(1) == 1)
        ? bitStreamReader.getBits(SIZE_X)
        : null;
    this.blockadeY = (bitStreamReader.getBits(1) == 1)
        ? bitStreamReader.getBits(SIZE_Y)
        : null;
    this.roadPassable = (bitStreamReader.getBits(1) == 1)
        ? (0 != bitStreamReader.getBits(SIZE_PASSABLE))
        : null;
  }
  ...
  public BitOutputStream toBitOutputStream() {
    ...
    if (this.roadBlockadeID != null) {
      bitOutputStream.writeBitsWithExistFlag(this.roadBlockadeID.getValue(),
          SIZE_BLOCKADEID);
    } else if (this.rawBlockadeID != -1) {
      bitOutputStream.writeBitsWithExistFlag(this.rawBlockadeID,
          SIZE_BLOCKADEID);
    } else {
    ...
    if (this.blockadeRepairCost != -1) {
      bitOutputStream.writeBitsWithExistFlag(this.blockadeRepairCost,
          SIZE_COST);
    } else {
      bitOutputStream.writeNullFlag();
    }
    ...
    if (this.roadPassable != null) {
      bitOutputStream.writeBitsWithExistFlag((this.roadPassable ? 1 : 0),
          SIZE_PASSABLE);
    } else {
      bitOutputStream.writeNullFlag();
    }
  ...
  public String getCheckKey() {
    return getClass().getCanonicalName() + " > road:"
        + this.getRoadID().getValue();
  }
}
