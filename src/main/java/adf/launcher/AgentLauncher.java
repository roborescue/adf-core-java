...
import adf.launcher.connect.ConnectorAmbulanceCentre;
import adf.launcher.connect.ConnectorAmbulanceTeam;
import adf.launcher.connect.ConnectorFireBrigade;
import adf.launcher.connect.ConnectorFireStation;
import adf.launcher.connect.ConnectorPoliceForce;
import adf.launcher.connect.ConnectorPoliceOffice;
...
public class AgentLauncher {
  ...
  public AgentLauncher(String... args) throws ClassNotFoundException, ClassCastException, InstantiationException,
      IllegalAccessException, NoSuchMethodException, InvocationTargetException {
    this.init(args);
  }

  private void init(String... args) throws ClassNotFoundException, ClassCastException, InstantiationException,
      IllegalAccessException, NoSuchMethodException, InvocationTargetException {
    this.initSystem();
  ...
  private void initConnector() throws ClassNotFoundException, ClassCastException, InstantiationException,
      IllegalAccessException, NoSuchMethodException, InvocationTargetException {
    // load AbstractLoader
  ...
  public void start() {
    ...
    List<Thread> threadList = this.connectors.stream().map(connector -> new Thread(() -> {
    ...
    ConsoleOutput
        .finish("Done connecting to server (" + connectedCount + " agent" + (connectedCount > 1 ? 's' : "") + ")");
