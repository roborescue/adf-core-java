...
public class CoreUpdater {
  ...
  public boolean updateCore() {
    ...
    String sourcePath = coreFile.getParentFile().getAbsolutePath()
        + File.separator + SOURCES_DIR + File.separator + SOURCES_FILE;
  ...
  private void download( String urlStr, String fileStr, boolean isUpdateEtag )
      throws Exception {
    ...
    InputStream dataInStream = new DataInputStream(
        connection.getInputStream() );
    OutputStream dataOutStream = new DataOutputStream(
        new BufferedOutputStream( new FileOutputStream( fileStr ) ) );
  ...
  private String createMD5Digest( File file )
