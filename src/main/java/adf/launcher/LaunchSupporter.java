...
public class LaunchSupporter {
  ...
  public void delegate( List<String> args ) {
    ...
    if ( args.contains( OPTION_MAKESCRIPTS ) ) {
      ...
      makeScript( compilerJavaHome, "precompute", "adf.sample.SampleLoader",
          "-t", "$1,$2,$3,$4,$5,$6", "-h", "$7", "-pre", "true" );
      makeScript( compilerJavaHome, "start", "adf.sample.SampleLoader", "-t",
          "$1,$2,$3,$4,$5,$6", "-h", "$7" );
      args.add( OPTION_MAKELAUNCHER );
      worked = true;
    }
    ...
    /*
     * if (args.contains(OPTION_AUTOCLASSPATH)) { removeOption(args,
     * OPTION_AUTOCLASSPATH); autoLoadDefaultClassPath(); }
     */

    if ( args.contains( OPTION_CHECK )
        && ( !args.contains( OPTION_NOCHECK ) ) ) {
      if ( !args.contains( OPTION_MAKESCRIPTS ) ) {
  ...
  private void printOptionList() {
    System.out
        .println( "* Please run following command when you update librarys." );
    System.out
        .println( "java -jar library/rescue/adf/adf-core.jar -makelauncher" );
    System.out.println( "" );
    System.out.println( "" );
    ...
    System.out
        .println( "-makescripts\t\t\t\tmake utility scripts(start.sh, etc.)" );
    ...
    // DIRECTORY_LIBRARY);
    System.out.println(
        "-autolc\t\t\t\t\tauto load loader class form " + DIRECTORY_BUILD );
    System.out.println( "-d [0|1]\t\t\t\tDebug flag" );
    System.out
        .println( "-check\t\t\t\t\tsimple agent class check (with -autocp)" );
  ...
  private void autoLoadDefaultClassPath() {
    ...
    String[] classPathArray = getClassPath( DIRECTORY_LIBRARY )
  ...
  private void makeLauncherScript( String javaHome ) {
    ...
    ConsoleOutput.info(
        "launcher." + ( isWindows ? "bat" : "sh" ) + " is made on " + workDir );
    String java = "java";
    if ( javaHome != null ) {
      ConsoleOutput.info( "JAVA_HOME directory: " + javaHome );
      javaHome = Pattern.compile( File.separator + "$" ).matcher( javaHome )
          .replaceFirst( "" );
    ...
    try {
      ...
      if ( !isWindows ) {
        try {
          ProcessBuilder processBuilder = new ProcessBuilder( "chmod", "a+x",
              "launcher.sh" );
  ...
  private void makeScript( String javaHome, String name, String... commands ) {
    String workDir = System.getProperty( "user.dir" );
    ConsoleOutput.info(
        name + "." + ( isWindows ? "bat" : "sh" ) + " is made on " + workDir );
  ...
  private void checkAgentClass( String javaHome ) {
    countAgentCheckWarning = 0;
    myCodeSource = this.getClass().getProtectionDomain().getCodeSource()
        .getLocation();
  ...
  private void checkAgentClass( String base, String path, String jdeps ) {
    ...
    if ( files != null ) {
      Arrays.sort( files,
          ( a, b ) -> (int) ( b.lastModified() - a.lastModified() ) );
      for ( File file : files ) {
        if ( file.isFile() ) {
          ...
          if ( filePath.endsWith( ".class" ) && !filePath.contains( "$" ) ) {
            ...
            String loaderClass = filePath
                .substring( base.length() + 1, filePath.length() - 6 )
                .replace( File.separator, "." );

            try {
              Class clazz = ClassLoader.getSystemClassLoader()
                  .loadClass( loaderClass );
              ...
              noStructureWarning = clazz
                  .isAnnotationPresent( NoStructureWarning.class );
                ...
                for ( Method method : clazz.getDeclaredMethods() ) {
                  if ( !( Modifier.isPrivate( method.getModifiers() ) ) ) {
                    ...
                    for ( int i = 0; ( i < method.getParameterTypes().length )
                      ...
                      Type genericParamType = method
                          .getGenericParameterTypes()[i];
                      if ( !( paramType.getName()
                          .equals( genericParamType.getTypeName() ) )
                          && genericParamType.getTypeName()
                              .split( "\\." ).length == 1 ) {
                    ...
                    if ( hasGenericsParam ) {
                      String regex = "^" + method.getReturnType().getName()
                          + ":" + method.getName() + ":";
                      for ( Class paramClass : method.getParameterTypes() ) {
                        regex += ".*,";
                      }
              ...
              if ( isAdfChild && methodList.size() > 0 ) {
                ConsoleOutput.warn( "Independent public method is exist in "
                    + loaderClass + " :" );
              ...
              for ( Field field : ClassLoader.getSystemClassLoader()
                  .loadClass( loaderClass ).getDeclaredFields() ) {
                if ( Modifier.isStatic( field.getModifiers() )
                    && !( Modifier.isFinal( field.getModifiers() ) )
                    && !( noStructureWarning ) ) {
                  ConsoleOutput.warn( "Variable static field is exist : "
                      + loaderClass + "." + field.getName() );
                  countAgentCheckWarning++;
                } else if ( isAdfChild
                    && Modifier.isPublic( field.getModifiers() ) ) {
                  ConsoleOutput.warn( "Public field is exist : " + loaderClass
                      + "." + field.getName() );
                  countAgentCheckWarning++;
                }
              }
              ...
              if ( !( noStructureWarning ) ) {
                ...
                try {
                  ...
                  String myPackage = loaderClass.substring( 0,
                      loaderClass.lastIndexOf( '.' ) );
                  ...
                  for ( String className : dependClasses ) {
                    if ( myPackage.indexOf( className.substring( 0,
                  ...
                  if ( sb.length() > 0 ) {
                    countAgentCheckWarning++;
                    ConsoleOutput.warn(
                        "Violation structure (invoke the parent package class) :" );
    ...
    if ( files != null ) {
      ...
      for ( File file : files ) {
        if ( file.isFile() ) {
          String filePath = file.getPath();
          if ( filePath.endsWith( ".class" ) && !filePath.contains( "$" ) ) {
            loaderClass = filePath
                .substring( base.length() + 1, filePath.length() - 6 )
                .replace( File.separator, "." );
            try {
              if ( ClassLoader.getSystemClassLoader().loadClass( loaderClass )
